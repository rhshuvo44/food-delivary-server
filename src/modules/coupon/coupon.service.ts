...
export class CouponService {
    ...
    static async updateCoupon(user: AuthUser, couponId: string, payload: UpdateCouponPayload) {
        ...
        await this.assertCouponAccess(user, coupon.restaurant?.ownerId, coupon.restaurantId);
        ...
        if (payload.usageLimit !== undefined && payload.usageLimit !== null && payload.usageLimit < coupon.usedCount) {
    ...
    static async applyCoupon(payload: ApplyCouponPayload) {
        ...
        if (coupon.restaurantId && payload.restaurantId && coupon.restaurantId !== payload.restaurantId) {
    ...
    private static async assertCouponAccess(user: AuthUser, ownerId?: string, restaurantId?: string | null) {
        if (user.role === 'ADMIN') {
            return;
        }
